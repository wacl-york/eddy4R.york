...
#' Modified to expose freqThsh (cutoff frequency for high pass filter as an argument)
...
#   Will Drysdale (2025-11-12)
#    Move freqThsh to function argument, remove code relating to outputing a plot and make changes to pass R CMD Check
##############################################################################################
...
def.lag2 <- function(
  ...
  if(length(stats::na.omit(cbind(refe, meas))) / length(refe) < 0.1) {
    ###
    ...
    refe <- stats::approx(x=zoo::index(refe), y=refe, xout=zoo::index(refe))[[2]]
    meas <- stats::approx(x=zoo::index(meas), y=meas, xout=zoo::index(meas))[[2]]

    #get rid of NAs at start and end
    naNot <- stats::na.omit(data.frame(refe=refe, meas=meas))
    refe <- naNot$refe
    meas <- naNot$meas
    rm(naNot)
    ...
    if(lagCnst == TRUE) {
      ...
      corr <- stats::ccf(refe, meas, lag.max = lagMax, plot = FALSE, na.action = stats::na.pass)
